...
class Settings_Joomlabridge_Record_Model extends Settings_Vtiger_Record_Model {

	const tableName = 'vtiger_joomla_fieldmaps';
	/**
	...
	public function getRecordLinks() {
		...
		$recordLinks = array(
				array(
						...
						'linklabel' => 'LBL_EDIT',
						'linkurl' => "javascript:Settings_Joomlabridge_List_Js.triggerEdit(event, '".$this->getEditViewUrl()."');",
						'linkicon' => 'icon-pencil'
				),
				array(
						'linktype' => 'LISTVIEWRECORD',
						'linklabel' => 'LBL_DELETE',
						'linkurl' => "javascript:Settings_Joomlabridge_List_Js.triggerDelete(event, '".$this->getDeleteUrl()."');",
						'linkicon' => 'icon-trash'
				)
		);
	...
	public function save() {
		...
		$params = array(
			...
			$this->get('vt_table'),
			$this->get('vt_field'),
			$this->get('push_to_joomla'),
			$this->get('pull_from_joomla'),
		...
		if ($id) {
			$query = 'UPDATE '.self::tableName.' SET jfunction = ?, jtable = ?, jfield = ?, vt_module = ?, vt_table = ?, vt_field = ?, push_to_joomla = ?, pull_from_joomla = ?, description = ?, isactive = ? WHERE id = ?';
			array_push($params, $id);
		} else {
			$query = 'INSERT INTO '.self::tableName.' (jfunction, jtable, jfield, vt_module, vt_table, vt_field, push_to_joomla, pull_from_joomla, description, isactive) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?)';
		}
